				...
				for frame in all(phone[phoneme]) do
					...
					for m=1,#c1 do
						...
						if m<4  and source==1 then --Don't include nasal filter
							cm[1] *= spk8_shift --f2
							f_goal *= spk8_shift --f2
						end	
						add(f_glide,shift*(f_goal-cm[1])/d) --shift: how quickly to shift pitch
						add(bw_glide,shift*(c2m[2]-cm[2])/d)
					end
					if  h_phone then  -- get hh cascade
						add(sounds,{440,2,0,1,0,1,pitch,c,f_glide,bw_glide,c2,v_stress,delta_f0})
						h_phone=false
					end
...
function speako8()
	function set_voice()
		w0=((5512.5/(spk8_pitch+delta_f0))+(w0 and w0*499 or 0))/(w0 and 500 or 1)
	end
	if #sounds >0 then
...
gs_update=
{
	...
	function() --menu
		...
		if (btnp()&0x00ff>=0x0010)then
			say'_/-1.67/_/g/-1.34/3/ow/-1.72/ih/-1.12/ng/_/-1.70/_/g/-1.34/3/ow/-1.72/ih/-1.12/ng/_/-1.70/-3/_/g/1.08/-3/ao/1.09/-3/n'
			_update=gs_update[1]
		end
	...
	function() -- menu
		...
		for i=1,#menu do
			if i==selection then
				...
				printc(menu[i],(i-1)*12+6,11)
				rect(30,(i-1)*12+3,90,(i-1)*12+13,11)
				print("⬅️",21,(i-1)*12+6,11)
				print("➡️",93,(i-1)*12+6,11)
			else
				printc(menu[i],(i-1)*12+6,5)
				rect(30,(i-1)*12+3,90,(i-1)*12+13,5)
			end	
		end
		printc("❎ play",119,11)	
	end
...
function _init()
	...
	spk8_if0=10 --hertz, how much speaker's pitch varies for high versus low vowels.
	spk8_intonation=10--20 hz for women, 10 for men, 0 for robotic monotone
	spk8_rate=1 --duration multiplier for voiced sounds. <below 1 faster, above 1 slower
	spk8_shift=1 --1 for man, 1.1 for woman
	spk8_pitch=140 --voicing pitch 140 man, 220 woman
	spk8_volume=1
	spk8_whisper=1 --1 normal, 2 whisper
	spk8_quality=.5--.1 creaky, 1 breathy,2 weak
	
	--say'_/hh/-1.56/ah/-1.07/l/-1.36/3/ow/-3/_/-1.03/-3/w/1.08/-3/er/1.65/-3/l/-1.64/-3/_/d/_/-1.11/3/ay/-1.02/m/-1.30/s/-1.64/_/p/-1.15/3/iy/-1.68/_/k/-1.13/3/ow/-1.46/3/ey/-1.71/_/t/_/-1.52/ah/-1.18/s/-1.64/_/p/-1.11/3/iy/-1.66/_/ch/-1.18/s/-1.41/3/ih/-1.18/n/-1.25/dh/-1.60/ah/-1.09/s/-1.47/ah/-1.09/s/-1.15/l/-1.16/3/ay/-1.66/_/b/-1.25/r/-1.63/eh/-1.09/r/-1.45/iy/f/-1.26/ao/-1.09/r/-1.62/_/p/-1.27/3/iy/-1.68/_/k/-1.55/ow/-1.19/-3/ey/-1.64/-3/_/t/_/1.11/ay/-1.67/_/k/-1.18/ae/-1.06/n/-1.18/s/-1.22/3/ey/m/-1.19/ow/-1.27/s/-1.72/_/t/-1.18/3/eh/-1.06/n/-1.67/iy/-1.09/th/-1.46/ih/-1.05/ng/-1.15/y/-1.26/uw/-1.07/-3/l/-1.00/-3/ay/-1.65/-3/_/k'
