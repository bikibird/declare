...
do
	...
	function say(speech)
		...
		for phoneme in all(phonemes) do
			...
			--elseif phoneme=="h" then --extra aspiration for terminal stop consonants
	...
	function vocals(options)
		spk8_pitch,spk8_rate,spk8_voicing,spk8_quality,spk8_intonation,spk8_if0,spk8_shift,spk8_bandwidth,spk8_aspiration,spk8_tilt=unpack(options)
	end	
	function speako8()
			...
			for i=0,127 do
				...
				if source==1 then	
					if t<open-1 then
						...
						x0+=a
						y0=x0*(1-spk8_tilt)+ y1*spk8_tilt
						y1=y0
					else
						x0,y1,y0=0,0,0
					end	
				...
				for k,resonator in pairs(cascade) do
					local f,bw=resonator[1],resonator[2]*(k>1 and k<4 and spk8_bandwidth or 1)\10+1 
