			...
			for frame in all(phone[phoneme]) do
				if phoneme=="hh" then
					...
					if (i>1) then  -- add second half of segment
						...
						c=c2
						add_sound()
						--add(sounds,{max(d2,1),source,frication,v1,v2,pitch,delta_f0,v_stress,c,c2})	
					...
					add_sound()
					--add(sounds,{max(d2,1),source,frication,v1,v2,pitch,delta_f0,v_stress,c,c2})--first half of phone
			...
			for i=0,127 do
				...
				if source ==3 then 
					...
					for k,resonator in pairs(cascade) do
						...
						--resonator[1]=cascade2[k][1]*.02+resonator[1]*.98
						--resonator[2]=cascade2[k][2]*.02+resonator[2]*.98
						--printh(k..": "..resonator[1].." counter: "..counter,"formants")
