			...
			for frame in all(phone[phoneme]) do
				if phoneme=="hh" then
					...
					if (i>1) then  -- add second half of segment
						...
						-- c has already made half the journey from c1 to c2.  
			...
			for i=0,127 do
				...
				if source ==3 then 
					...
					for k,resonator in pairs(cascade) do
						...
						f=resonator[1]
						resonator[1]=c2k[1]*.02+f*.98
						bw=resonator[2]
						resonator[2]=c2k[2]*.02+bw*.98
					end
